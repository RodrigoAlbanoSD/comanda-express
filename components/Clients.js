...
import { TouchableWithoutFeedback, SafeAreaView, Text, View, Image, TextInput, FlatList, TouchableOpacity } from 'react-native';
...
const Clients = (props) => {
  ...
  return (
    <View style={props.onActive === 1 ? "" : "hideComponent"}>
      ...
      <View>
        {props.clients?.map((client) => (
          ((inputValues.client === '' ? true : inputValues.client === client.name ? true : false)) ?
            <SafeAreaView key={"ClientCardWrapperSafeareaView" + client.email} style={styles.cardsContainer}>


              <TouchableWithoutFeedback
                key={"ClientCardWrapper" + client.email}
                onPress={(props.activeClient.email === client.email) ? () => {
              ...
              >
                <View>
                  <View key={"CardNav" + client.email} style={{
                    flexDirection: 'row',
                    margin: "2%",
                    justifyContent: 'space-between'
                  }}>
                    <Text style={[styles.header, { marginLeft: "3%" }]}>{client.name}</Text>
                  </View>
                  ...
                  <View key={"ClientCard" + client.email} style={styles.card}>
                    <View style={{
                      ...
                      <View >
                        <Button
                          style={client.email === props.activeClient?.email ? styles.btnCardActive : styles.btnCard}
                          onPress={() => {
                      ...
                      <View style={{ marginTop: "6%", marginLeft: "2%"}}>
                        <Text style={[styles.cardText, { marginBottom: "5%" }]}>{client.email}</Text>
                        <Text style={[styles.cardText, { marginBottom: "5%" }]}>{client.address}</Text>
                        <Text style={[styles.cardText, { marginBottom: "5%" }]}>{client.tel}</Text>
                        {client.notes !== "" ? <Text style={[styles.cardText, { marginBottom: "5%" }]}>{client.notes}</Text> : ""}
                      </View>
                    </View>
