...
import { SafeAreaView, Text, View, TouchableWithoutFeedback, Image, TextInput, FlatList, TouchableOpacity } from 'react-native';
        ...
        <View style={props.onActive === 8 ? "" : "hideComponent"}>
            ...
            {props.products?.map((product, index) => (
                <View key={"ProductCard" + product.id + index}>
                    ...
                    {(inputValues.productFilter === '' ? true : inputValues.productFilter === product.name ? true : false) ?
                        //Container for every client          
                        <SafeAreaView style={styles.cardsContainer} key={"ProductsCardWrapperSafeareaView" + product.id + index}>

                            <TouchableWithoutFeedback
                                key={"ProductsCardWrapper" + product.id + index}
                                onPress={(props.productActive?.name === product.name) ? () => props.onSelectedProduct({}) : () => props.onSelectedProduct(product)}
                            >
                                <View key={"ProductCard" + product.id} >
                                    {/*client name*/}
                                    <Text style={[styles.header, { margin: "1%", marginLeft: "3%" }]}>{product.name}</Text>
                                    ...
                                    <View style={[styles.card, {
                                        ...
                                        <View key={"CardNav" + product.name}>
                                            <Button
                                                style={props.productActive?.name === product.name ? styles.btnCardActive : styles.hideComponent}
                                                onPress={() => {
                                                    props.onClick(10);
                                                }}
                                                disabled={props.productActive?.name === product.name ? false : true}
                                            >
                                                <Image
                                        ...
                                        <Text style={[styles.cardText, { marginTop: "3%", marginLeft: "2%" }]}>{product.desc}</Text>
                                    </View>
                                </View>
                            </TouchableWithoutFeedback>
                        </SafeAreaView>
