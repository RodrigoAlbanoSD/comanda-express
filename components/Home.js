    ...
    <View style={props.onActive === 0 ? "" : "hideComponent"}>
      ...
      {props.clients?.map((client) => (
        <View key={"HomeCard" + client.email}>
          {client.hasOwnProperty("orders") &&
          client.orders?.find((o) => o.isActive === true) &&
          (inputValues.clientFilter === ""
            ? true
            : inputValues.clientFilter === client.name
            ? true
            : false) ? (
            //Container for every client
            <SafeAreaView
              key={"SafeAreaView" + client.email}
              style={styles.cardsContainer}
            >
              {/*client name*/}
              <Text
                style={[styles.header, { marginLeft: "3.5%", marginTop: "3%" }]}
              >
                {client.name}
              </Text>
                    ...
                    >
                      ...
                      >
                        <View key={"CardNav" + order.id} style={styles.carNav}>
                          <Text style={[styles.header, { marginTop: "3%" }]}>
                            REF: {order.id}
                          </Text>
                          <View
                            style={{ flexDirection: "row-reverse", gap: "15%" }}
                          >
                            <Button
                              style={
                                props.order?.id === order.id
                              ...
                              disabled={
                                props.order?.id === order.id ? false : true
                              }
                            >
                              <Image
                        ...
                        <View key={"OrderCard" + props.order.id}>
                          <View
                            style={[styles.formRow, { marginBottom: "1%" }]}
                          >
                            {order.warehouse ? (
                              <Text
                                style={{
                          ...
                          <View
                            style={[styles.formRow, { borderBottomWidth: 1 }]}
                          >
                            {order.desc ? (
                              <Text style={styles.cardText}>{order.desc} </Text>
                            ) : (
                              ""
                            )}
                          </View>
                          ...
                          <View style={[styles.formRow, { marginBottom: 0 }]}>
                            <Text style={[styles.cardLabel, { flex: 3 }]}>
                              Produto{" "}
                            </Text>
                          ...
                          <View style={{ borderBottomWidth: 1 }}>
                            {order.product?.map((product, index) => (
                              ...
                              >
                                ...
                                <View
                                  key={"PriceAmountHome" + index}
                                  style={[styles.formRow, { flex: 2 }]}
                                >
